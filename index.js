...
function concatenateStrings(array, subString) {
  ...
  //Перевіряємо чи містить рядок підрядок
   let containsSubstring = string.includes(subString);
  // Виводимо результат
...
function swapMinMax(array) {
  // Ініціалізуємо дві змінні (min і max) з першим елементом масиву.
  let min = array[0];
  let max = array[0];
  // Ініціалізуємо дві змінні (minIndex і maxIndex) з нульовим значенням.
  let minIndex = 0;
  let maxIndex = 0;

  // Перевіряємо кожен елемент масиву, щоб знайти мінімальне та максимальне значення, а також їхні індекси.
  ...
  // Повертаємо масив, в якому мінімальне та максимальне значення обміняні місцями.
  return array;
}
